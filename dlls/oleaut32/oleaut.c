...
static HRESULT WINAPI PSOAInterface_QueryInterface(IPSFactoryBuffer *iface, REFIID iid, void **obj)
{
...
static ULONG WINAPI PSOAInterface_AddRef(IPSFactoryBuffer *iface)
{
    return 2;
}

static ULONG WINAPI PSOAInterface_Release(IPSFactoryBuffer *iface)
{
    return 1;
}

static HRESULT WINAPI PSOAInterface_CreateProxy(IPSFactoryBuffer *iface,
    IUnknown *outer, REFIID iid, IRpcProxyBuffer **proxy, void **obj)
{
    ITypeInfo *typeinfo;
    HRESULT hr;
    ...
    hr = CreateProxyFromTypeInfo(typeinfo, outer, iid, proxy, obj);
    if (FAILED(hr))
...
static HRESULT WINAPI PSOAInterface_CreateStub(IPSFactoryBuffer *iface, REFIID iid,
    IUnknown *server, IRpcStubBuffer **stub)
{
    ITypeInfo *typeinfo;
    HRESULT hr;
...
static const IPSFactoryBufferVtbl PSOAInterface_vtbl =
{
    PSOAInterface_QueryInterface,
    PSOAInterface_AddRef,
    PSOAInterface_Release,
    PSOAInterface_CreateProxy,
    PSOAInterface_CreateStub,
};

static IPSFactoryBuffer PSOAInterface = { &PSOAInterface_vtbl };
    ...
    IUnknown *outer, REFIID iid, IRpcProxyBuffer **proxy, void **obj)
{
    ...
    if (IsEqualIID(iid, &IID_IDispatch))
    {
        ...
        return IPSFactoryBuffer_CreateProxy(&PSOAInterface, outer, iid, proxy, obj);
}
    ...
    REFIID iid, IUnknown *server, IRpcStubBuffer **stub)
{
    ...
    if (IsEqualIID(iid, &IID_IDispatch))
    {
        ...
        return IPSFactoryBuffer_CreateStub(&PSOAInterface, iid, server, stub);
}
...
HRESULT WINAPI DllGetClassObject(REFCLSID rclsid, REFIID iid, LPVOID *ppv)
{
    *ppv = NULL;
    ...
    if (IsEqualGUID(rclsid, &CLSID_PSOAInterface))
        return IPSFactoryBuffer_QueryInterface(&PSOAInterface, iid, ppv);
